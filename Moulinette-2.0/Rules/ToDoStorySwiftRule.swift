...
final class ToDoStorySwiftRule: SwiftRule {
    ...
    func run() -> AuditGrade {
        for (fileName, fileComponents) in projectData.applicationComponents {
            ToDoStorySwiftRule.checkFileLine(fileName: fileName, fileComponents: fileComponents) { (comment, line) in
...
private extension ToDoStorySwiftRule {

    static func checkFileLine(fileName: String,
                              fileComponents: [String],
                              lineCompletion:((_ comment: String, _ line: String) -> ())) {
        var comment = ""
